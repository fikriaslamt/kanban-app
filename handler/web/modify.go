...
func (a *modifyWeb) AddTask(w http.ResponseWriter, r *http.Request) {
	...
	var AddTask = path.Join("views", "main", "add-task.html")
...
func (a *modifyWeb) AddTaskProcess(w http.ResponseWriter, r *http.Request) {
	...
	if respCode == 201 {
		...
		http.Redirect(w, r, "/category/add?category="+category, http.StatusSeeOther)
	}
}
...
func (a *modifyWeb) AddCategory(w http.ResponseWriter, r *http.Request) {
	...
	var header = path.Join("views", "general", "header.html")
	var AddCategory = path.Join("views", "main", "add-category.html")
	tmpl, err := template.ParseFS(a.embed, AddCategory, header)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (a *modifyWeb) UpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: answer here
	var header = path.Join("views", "general", "header.html")
	var UpdateTask = path.Join("views", "main", "update-task.html")
