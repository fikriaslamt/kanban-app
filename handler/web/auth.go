...
func (a *authWeb) Login(w http.ResponseWriter, r *http.Request) {
	...
	var login = path.Join("views", "auth", "login.html")
	tmpl, err := template.ParseFS(a.embed, login, header)
	if err != nil {
...
func (a *authWeb) Register(w http.ResponseWriter, r *http.Request) {
	var header = path.Join("views", "general", "header.html")
	var register = path.Join("views", "auth", "register.html")
	tmpl, err := template.ParseFS(a.embed, register, header)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
